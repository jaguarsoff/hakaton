    ...
    if($authorizationHeader) {
        ...
        if($decodedToken) {
            ...
            if($userData) {
                ...
                // Добавляем информацию о роли пользователя (модератор или нет)
                $isModerator = isset($firstUser['isModerator']) ? $firstUser['isModerator'] : false;

                // Возвращаем нужные данные, включая информацию о роли
                echo json_encode([
                    'status' => 'success',
